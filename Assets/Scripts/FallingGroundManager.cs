...
public class FallingGroundManager : MonoBehaviour
{
    ...
    public PlayerController4 playerController4; // Reference to the Player Controller
    ...
    void Update()
    {
        ...
        {
            if ((playerController.transform.position.z > fallingGrounds1[i].transform.position.z + 1) || (playerController4.transform.position.z > fallingGrounds1[i].transform.position.z + 1))// Checking whether the player is ahead of the cube so that it can't be outrun
            {
        ...
        if (playerController.checkpointReached == 1 || playerController4.checkpointReached == 1) // Checking whether the player has reached the 1st checkpoint
        {
        ...
        if (playerController.checkpointReached == 2 || playerController4.checkpointReached == 2) // Checking whether the player has reached the 2nd checkpoint
        {
            for (int i = 0; i < fallingGrounds3.Length; i++) // Looping through the array of cubes (ground)
            {
                if ((playerController.transform.position.z > fallingGrounds3[i].transform.position.z - 1) || (playerController4.transform.position.z > fallingGrounds3[i].transform.position.z - 1))// Checking when the player approaches the next cube
                {
                ...
                if ((playerController.transform.position.z > fallingGrounds3[i].transform.position.z + 1) || (playerController4.transform.position.z > fallingGrounds3[i].transform.position.z + 1))// Checking whether the player has passed the previous cube
                {
